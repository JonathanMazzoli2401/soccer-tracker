(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function e(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(a){if(a.ep)return;a.ep=!0;const r=e(a);fetch(a.href,r)}})();function c(t,s){t.innerHTML=s}function i(t,s){const e=document.querySelector(t);e&&(e.textContent=s)}function g(){return`
  ...
  `:'<p class="muted">No matches available for this team yet.</p>'}function S(t){var a;const s=(a=t.score)==null?void 0:a.fullTime;if(!s)return"";const e=s.home??"-",n=s.away??"-";return`• FT: ${e}-${n}`}const p="soccer-tracker:favorites";function T(){try{return JSON.parse(localStorage.getItem(p))||[]}catch{return[]}}function E(t){const s=T(),n=s.some(a=>a.type===t.type&&String(a.id)===String(t.id))?s.filter(a=>!(a.type===t.type&&String(a.id)===String(t.id))):[...s,t];return localStorage.setItem(p,JSON.stringify(n)),n}const L="https://www.thesportsdb.com/api/v1/json",D="123";async function B(t,s={}){const e=new URL(`${L}/${D}${t}`);Object.entries(s).forEach(([a,r])=>{r!=null&&r!==""&&e.searchParams.set(a,r)});const n=await fetch(e.toString());if(!n.ok){const a=await n.text();throw new Error(`TheSportsDB error ${n.status}: ${a}`)}return n.json()}async function k(t){return((await B("/searchteams.php",{t})).teams||[]).map(n=>({id:n.idTeam,name:n.strTeam,country:n.strCountry,badge:n.strTeamBadge}))}async function P(t){const e=`https://www.thesportsdb.com/api/v1/json/123/searchplayers.php?p=${encodeURIComponent(t)}`,n=await fetch(e);if(!n.ok)throw new Error("TheSportsDB player search failed");return((await n.json()).player||[]).map(r=>({id:r.idPlayer,name:r.strPlayer,nationality:r.strNationality,team:r.strTeam,thumb:r.strThumb,description:r.strDescriptionEN}))}const M="https://ancient-paper-fac2.jonathanmazzoli25.workers.dev/v4",N=M;function j(t,s={}){const e=new URL(N+t,window.location.origin);return Object.entries(s).forEach(([n,a])=>{a!=null&&a!==""&&e.searchParams.set(n,a)}),e.toString()}async function q(t,s={}){const e=j(t,s),a=await fetch(e,{headers:{"X-Auth-Token":"3701adedeee64479b255cd46efb7c2ca"}});if(!a.ok){const r=await a.text();throw new Error(`football-data.org error ${a.status}: ${r}`)}return a.json()}async function m(t,{status:s="SCHEDULED",limit:e=10}={}){return(await q(`/teams/${t}/matches`,{status:s,limit:e})).matches||[]}const x={arsenal:57,chelsea:61,liverpool:64,"manchester city":65,"manchester united":66,tottenham:73,barcelona:81,"real madrid":86};function I(t){return x[t.trim().toLowerCase()]||null}let f=[],h=[];function O(){const s=(window.location.hash||"#/").replace("#/","").split("/").filter(Boolean);return{route:s[0]||"",id:s[1]||""}}function R(){const t=document.querySelector("#app");c(t,g()),t.insertAdjacentHTML("beforeend",$());const s=document.querySelector("#searchForm"),e=document.querySelector("#results");s.addEventListener("submit",async n=>{n.preventDefault();const a=document.querySelector("#searchType").value,r=document.querySelector("#searchQuery").value.trim();e.innerHTML="",i("#status","Loading...");try{let o=[];a==="teams"?(o=await k(r),f=o):(o=await P(r),h=o),e.innerHTML=v(o,a),i("#status",`Results: ${o.length}`)}catch(o){console.error(o),i("#status","Error loading results."),e.innerHTML=`<p class="error">${o.message}</p>`}}),e.addEventListener("click",n=>{const a=n.target.closest("[data-team-id]");if(a){window.location.hash=`#/team/${a.dataset.teamId}`;return}const r=n.target.closest("[data-player-id]");r&&(window.location.hash=`#/player/${r.dataset.playerId}`)}),t.addEventListener("click",n=>{const a=n.target.closest("button[data-action='fav']");a&&(E({type:a.dataset.type,id:a.dataset.id,name:a.dataset.name}),a.textContent=a.textContent==="☆"?"★":"☆")})}async function H(t){const s=document.querySelector("#app"),e=f.find(n=>String(n.id)===String(t));if(!e){c(s,`<section class="card">
        <a href="#/" class="muted small">← Back</a>
        <p class="muted">Team not found. Search again.</p>
      </section>`);return}c(s,'<section class="card"><p class="muted">Loading team details...</p></section>');try{const n=I(e.name);let a=[],r=[];n&&(a=await m(n,{status:"SCHEDULED",limit:5}),r=await m(n,{status:"FINISHED",limit:5})),c(s,b(e,a,r))}catch(n){console.error(n),c(s,`<section class="card">
        <a href="#/" class="muted small">← Back</a>
        <p class="error">${n.message}</p>
      </section>`)}}function A(t){const s=document.querySelector("#app"),e=h.find(n=>String(n.id)===String(t));if(!e){c(s,`<section class="card">
        <a href="#/" class="muted small">← Back</a>
        <p class="muted">Player not found. Search again.</p>
      </section>`);return}c(s,w(e))}function y(){const{route:t,id:s}=O();if(t==="team"&&s){H(s);return}if(t==="player"&&s){A(s);return}R()}window.addEventListener("hashchange",y);y();
